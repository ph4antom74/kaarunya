...
const CreateCenter=({ navigation }) => {
  ...
  useEffect(()=>{
    ...
    return (
      ...
      <Text style={{backgroundColor:'#fff',padding:20,fontSize:15}}>Center Id:{value.centerId}</Text>
      <Text style={{backgroundColor:'#fff',padding:20,fontSize:15}}>Center Name:{value.centerName}</Text>
      <Text style={{backgroundColor:'#fff',padding:20,fontSize:15}}>Email:{value.email}</Text>
      <Text style={{backgroundColor:'#fff',padding:20,fontSize:15}}>Uid:{value.uid}</Text>
      
    </CollapseBody>)})}
    <View style={{alignContent:'center',alignItems:'center',}}>
                <TouchableOpacity   style={styles.CCButton}  onPress={()=> navigation.navigate('ProfileScreen')}>
