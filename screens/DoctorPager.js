...
const DoctorPager = ({navigation,route}) => {
  ...
  <ScrollView>  
        ...
        <View style={{flexDirection:'row'}}>
         <Text style={{marginRight:'5%',fontSize:20}}>Name:{route.params.value.name}</Text>
         <Text style={{fontSize:20}}>Age:{route.params.value.age}</Text>
      </View>
        {
    ...
    <CollapseHeader>
      <View style={{backgroundColor:'#2c6b82',padding:10}}>
        <Text style={{color:'#fff',padding:10}}>Date of Visit:{value.date}</Text>
      </View>
    </CollapseHeader>
    <CollapseBody>
